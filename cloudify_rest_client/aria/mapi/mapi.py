...
class RESTMAPI(api.ModelAPI):
    ...
    def __init__(self, host, port, api, *args, **kwargs):
        super(RESTMAPI, self).__init__(*args, **kwargs)
        self._host = host
        self._port = port
        self._api = api
        self._endpoint = '{self._host}:{self._port}/{self._api}'.format(
            self=self)
        ...
        self._engine = type(
            'MockSession', (object,), {'dispose': lambda *a, **kw: None})()
    ...
    def _wrap(self, value, attribute_path):
        ...
        if isinstance(value, dict):
            ...
            wrapper = wrappers.ObjectWrapper
            kw = dict(obj=value)
